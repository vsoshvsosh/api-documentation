...
Кроме того, шаблоны могут содержать отслеживающие изображения, чтобы Ruphish знал, когда пользователь открывает электронное письмо.
...
Используя конечную точку [Импорт электронного письма](templates.md#import-template), вы можете взять необработанное электронное письмо и разобрать его как действительный шаблон Ruphish.
{% endhint %}
...
Ruphish предоставляет возможность парсить существующее электронное письмо для использования в качестве шаблона. Это облегчает перепрофилирование легитимных электронных писем для ваших фишинговых оценок.
...
При установке атрибута `convert_links` в значение `true`, Ruphish автоматически изменит все ссылки в электронном письме на `{{.URL}}`.
